...
CreateMonopoles <- function(){
 ...
 for(heart in c(paste0("0",c(1:9)),10:12,14:20,22:24)){
  #for(heart in c("01","02","03")){
      AT_apex <- read.table(paste0("/media/crg17/Seagate Backup Plus Drive/CT_cases/HF_case",heart,"/simulations/multipole/eikonal_scarred/BiV.RV_endo.apex/vm_act_seq.dat"))
    for(vein in c("AN","AL","LA","PL","PO")){
      for(electrode in c(1:8)){
      AT <- read.table(paste0("/media/crg17/Seagate Backup Plus Drive/CT_cases/HF_case",heart,"/simulations/multipole/eikonal_scarred/",vein,"_",electrode,"/vm_act_seq.dat"))
      
      AT_res <- pmin(AT_apex,AT)
      
      directory <- paste0("/data/SA_multipole/scarred/HF/",heart)
...
CreateDipoles <- function(which_cases="both"){
  require(jjb)
  
  if(which_cases == "both" || which_cases == "h"){
  for(heart in c(paste0("0",c(1:9)),10:20)){
    print(paste0("Working on case ",heart))
    files_vec <-  list.files(paste0("/data/SA_multipole/midseptum/h/",heart))
    for (vein in c("AN","AL","LA","PL","PO")) {
      ...
      for(i in c(1:ncol(indices))){
        AT1 <- read.table(paste0("/data/SA_multipole/midseptum/h/",heart,"/",files_vein[indices[1,i]]))
        AT2 <- read.table(paste0("/data/SA_multipole/midseptum/h/",heart,"/",files_vein[indices[2,i]]))
        ...
        outname <- paste0("h",heart,"_",vein,"_",merge_design(c(name1,name2)),".dat")
  ...
  if(which_cases == "both" || which_cases == "HF"){
  for(heart in c(paste0("0",c(1:9)),10:12,14:20,22:24)){
    files_vec <-  list.files(paste0("/data/SA_multipole/scarred/HF/",heart))
    for (vein in c("AN","AL","LA","PL","PO")) {
      files_vein <- files_vec[which(grepl(vein,files_vec))]
      indices <- combn(length(files_vein),2)
      for(i in c(1:ncol(indices))){
        AT1 <- read.table(paste0("/data/SA_multipole/scarred/HF/",heart,"/",files_vein[indices[1,i]]))
        AT2 <- read.table(paste0("/data/SA_multipole/scarred/HF/",heart,"/",files_vein[indices[2,i]]))
        ...
        outname <- paste0("HF",heart,"_",vein,"_",merge_design(c(name1,name2)),".dat")
