classdef MeshType < handle
    ...
    methods(Access = public)
        ...
        function obj = MeshType(npoints,ntriangles)
            if(nargin > 0)
                if(nargin ==1 )
                    ...
                    obj.Area = npoints.Area;
                    obj.centres = npoints.centres;
                    ...
                    obj.Area =zeros(ntriangles,1);
                    obj.centres = zeros(ntriangles,3);
        ...
        function calcualteArea(obj)
            % This function calcautles the area of each triangle.
            if size(obj.triangles,2)~=3
                disp('MeshType::invertOrientation -- ERROR: this method only works on triangular meshes' );
                return;
            end
            obj.Area = zeros(1,obj.ntriangles);
            obj.centres= zeros(obj.ntriangles,3);
            for index = 1:obj.ntriangles
                pt1 = obj.points(obj.triangles(index,1),:);
                pt2 = obj.points(obj.triangles(index,2),:);
                pt3 = obj.points(obj.triangles(index,3),:);
                v12 = bsxfun(@minus, pt2, pt1);
                v13 = bsxfun(@minus, pt3, pt1);
                v = cross(v12, v13, 2);
                obj.Area(index)  = sqrt(sum(v.*v, 2))/ 2;
                obj.centres(index,:) = (pt1+pt2+pt3)/3;
            end
        end      
